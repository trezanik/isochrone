...
#if 0
	if ( _gui_interactions.active_workspace == my_wksp->GetWorkspace()->GetID() )
	{
		...
		if ( _gui_interactions.save_current_workspace )
		{
			...
			my_wksp_data->settings[settingname_link_defaultmethod] = core::TConverter<size_t>::ToString(my_nodegraph.settings.link_default_method);
...
)
{
	...
	//ImGui::Text("Node : %s", node->GetID().GetCanonical());
	ImGui::Spacing();
...
void
ImGuiWkspTopology::DrawServiceManagement()
{
	using namespace trezanik::core;
...
// where to store these settings so they can be loaded? needs another public settings struct
	
	auto validate_rgba = [](std::pair<const std::string, std::string>& pair, ImU32 val) {
		size_t  v = core::TConverter<size_t>::FromString(pair.second);
		if ( v > UINT32_MAX )
		{
			TZK_LOG_FORMAT(LogLevel::Warning, "Ignoring invalid setting supplied for '%s': %s", pair.first.c_str(), pair.second.c_str());
			return false;
		}
		val = static_cast<ImU32>(v);
		return true;
	};
	...
	{
#if 1
		my_wksp->ApplySetting(setting.first.c_str(), setting.second.c_str());
#else
		ImU32  col = 0;

		switch ( TZK_RUNTIME_HASH(setting.first.c_str()) )
		{
		...
		case cth_grid_colour_background:
			if ( validate_rgba(setting, col) )
			{
				my_nodegraph.settings.grid_style.colours.background = static_cast<ImU32>(col);
				my_nodegraph.GetCanvas().configuration.colour = my_nodegraph.settings.grid_style.colours.background;
			}
			break;
		case cth_grid_colour_link:
			if ( validate_rgba(setting, col) )
				my_nodegraph.settings.grid_style.colours.link = static_cast<ImU32>(col);
			break;
		case cth_grid_colour_origin:
			if ( validate_rgba(setting, col) )
				my_nodegraph.settings.grid_style.colours.origins = static_cast<ImU32>(col);
			break;
		case cth_grid_colour_primary:
			if ( validate_rgba(setting, col) )
				my_nodegraph.settings.grid_style.colours.primary = static_cast<ImU32>(col);
			break;
		case cth_grid_colour_secondary:
			if ( validate_rgba(setting, col) )
				my_nodegraph.settings.grid_style.colours.secondary = static_cast<ImU32>(col);
			break;
		case cth_grid_colour_selector:
			if ( validate_rgba(setting, col) )
				my_nodegraph.settings.grid_style.colours.selector_rect = static_cast<ImU32>(col);
			break;
		case cth_grid_draw:
			my_nodegraph.settings.grid_style.draw = core::TConverter<bool>::FromString(setting.second.c_str());
			break;
		case cth_grid_draworigin:
			my_nodegraph.settings.grid_style.draw_origin = core::TConverter<bool>::FromString(setting.second.c_str());
			break;
		case cth_grid_size:
...
#endif
/// @todo cth for loading, also move this out into ImGuiWorkspace, more intuitive location (not topology-only based)
...
void
ImGuiWkspTopology::ServiceManagementSelection(
	SvcMgmtSwitch what
)
{
	switch ( what )
